...
cmp.setup {
  ...
  mapping = {
    ...
    ["<C-Space>"] = cmp.mapping(cmp.mapping.complete(), { "i", "c" }), -- Open completion suggestions
    ["<C-y>"] = cmp.config.disable, -- Disable default CTRL + y behaviour
    -- Close completions suggestions
    ["<C-e>"] = cmp.mapping { 
      i = cmp.mapping.abort(),
      c = cmp.mapping.close(),
    },
  ...
  -- Sources have an order of precedence. Here, snippets will be suggested, then butter, then path
  sources = {
    { name = "nvim_lsp" },
    { name = "luasnip" },
    { name = "buffer" },
